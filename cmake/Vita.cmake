...
macro(vita_create_self target source)
  ...
  if(TARGET ${source})
    set(sourcepath ${EXECUTABLE_OUTPUT_PATH}/${source})
  else()
  ...
  add_custom_command(OUTPUT ${EXECUTABLE_OUTPUT_PATH}/${sourcefile}.velf
      COMMAND ${VITA_ELF_CREATE} ${VITA_ELF_CREATE_FLAGS} ${sourcepath} ${EXECUTABLE_OUTPUT_PATH}/${sourcefile}.velf
    DEPENDS ${sourcepath}
    COMMENT "Converting to Sony ELF ${sourcefile}.velf" VERBATIM
  )

  set(self_outfile ${EXECUTABLE_OUTPUT_PATH}/${target}.out)

  ## SELF command
  separate_arguments(VITA_MAKE_FSELF_FLAGS)
  add_custom_command(OUTPUT ${self_outfile}
      COMMAND ${VITA_MAKE_FSELF} ${VITA_MAKE_FSELF_FLAGS} ${EXECUTABLE_OUTPUT_PATH}/${sourcefile}.velf ${self_outfile}
      DEPENDS ${EXECUTABLE_OUTPUT_PATH}/${sourcefile}.velf
  ...
  add_custom_target(${target}
    ...
    COMMAND ${CMAKE_COMMAND} -E copy ${self_outfile} ${EXECUTABLE_OUTPUT_PATH}/${target}
  )
...
macro(vita_create_vpk target titleid eboot)
  ...
  if(TARGET ${eboot})
    set(sourcepath ${EXECUTABLE_OUTPUT_PATH}/${eboot})
  else()
  ...
  add_custom_command(OUTPUT ${EXECUTABLE_OUTPUT_PATH}/${target}_param.sfo
    COMMAND ${VITA_MKSFOEX} ${VITA_MKSFOEX_FLAGS} ${vita_create_vpk_NAME} ${EXECUTABLE_OUTPUT_PATH}/${target}_param.sfo
    DEPENDS ${sourcepath}
    COMMENT "Generating param.sfo for ${target}"
  )

  set(vpk_outfile ${EXECUTABLE_OUTPUT_PATH}/${target}.out)

  ## VPK command
  separate_arguments(VITA_PACK_VPK_FLAGS)
  add_custom_command(OUTPUT ${vpk_outfile}
    COMMAND ${VITA_PACK_VPK} ${VITA_PACK_VPK_FLAGS} -s ${EXECUTABLE_OUTPUT_PATH}/${target}_param.sfo -b ${sourcepath} ${vpk_outfile}
    DEPENDS ${EXECUTABLE_OUTPUT_PATH}/${target}_param.sfo
    DEPENDS ${sourcepath}
    DEPENDS ${resources}
  ...
  add_custom_target(${target}
    ALL
    DEPENDS ${vpk_outfile}
    COMMAND ${CMAKE_COMMAND} -E copy ${vpk_outfile} ${EXECUTABLE_OUTPUT_PATH}/${target}
  )
